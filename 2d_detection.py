...
def save_keypoints_json(json_path, video_name, frame_number, fps, keypoints, confidence, width, height):
    """ 儲存關鍵點資料到 json """
    landmarks_data = {
...
def save_empty_json(json_path, video_name, frame_number, fps, width, height):
            ...
            if len(all_boxes) > 0:
                ...
                for keypoints, max_vals, person_id in zip(pose_preds, confidence, all_ids):
                    ...
                    save_keypoints_json(json_out, f'{video_name}.mp4',
                                    count, fps, keypoints, max_vals, frame_w, frame_h)
            
            # 處理缺失的 json
            if id1_data is None:
                save_empty_json(os.path.join(id1_dir, json_filename),
                            f'{video_name}.mp4', count, fps, frame_w, frame_h)
            if id2_data is None:
